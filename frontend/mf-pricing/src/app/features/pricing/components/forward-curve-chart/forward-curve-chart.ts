...
export class ForwardCurveChartComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get<any>('/api/v1/pricing/forward-curve').subscribe({
      next: (data) => {
        // Fallback to defaults if backend not ready, but we perform real fetch!
        const months = data.months || ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
        const pricesSE = data.pricesSE || [120, 115, 110, 105, 95, 90, 85, 90, 95, 100, 110, 115];
        const pricesS = data.pricesS || [125, 120, 115, 110, 100, 95, 90, 95, 100, 105, 115, 120];
